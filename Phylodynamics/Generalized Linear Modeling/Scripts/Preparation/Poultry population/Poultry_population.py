...
file_path = "estat_apro_ec_poula.tsv"
df = pd.read_csv(file_path, sep='\t')

df[['freq', 'hatchitm', 'animals', 'unit', 'geo_code']] = df.iloc[:, 0].str.split(',', expand=True)
...
country_sum.rename(columns={'avg_2016_2024': 'poultry_population'}, inplace=True)
...
cluster_summary = merged_with_cluster.groupby('GeoCluster', as_index=False)['poultry_population'].sum()
cluster_summary.rename(columns={'poultry_population': 'poultry_population'}, inplace=True)

cluster_summary.to_csv("GeoCluster_poultry_population.tsv", sep='\t', index=False)
