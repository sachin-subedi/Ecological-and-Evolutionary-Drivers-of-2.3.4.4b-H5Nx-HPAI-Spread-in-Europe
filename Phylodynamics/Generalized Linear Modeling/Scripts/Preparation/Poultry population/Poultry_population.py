...
for code, label in [("NO", "Norway"), ("CH", "Switzerland")]:
    if code not in country_sum["country_code"].values:
        row = pd.DataFrame([{"country_code": code, "poultry_population": 0.0000001}])
...
hc_summary = (
    ...
    .groupby("HC_Cluster", as_index=False)["poultry_population"]
    .sum()
    .rename(columns={"poultry_population": "Poultry_Population"})
)

output_file = "HC_GeoCluster_Poultry_Population.tsv"
