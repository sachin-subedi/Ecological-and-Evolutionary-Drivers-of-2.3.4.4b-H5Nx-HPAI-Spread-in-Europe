...
for code, label in [("NO", "Norway"), ("CH", "Switzerland")]:
    if code not in country_sum["country_code"].values:
        row = pd.DataFrame([{"country_code": code, "livestock_density_index": 0.0000001}])
...
# ── STEP 12: Summarise by HC_Cluster ─────────────────────────────────────────
hc_summary = (
    merged
    .dropna(subset=["HC_Cluster"])
    .groupby("HC_Cluster", as_index=False)["livestock_density_index"]
    .sum()
    .rename(columns={"livestock_density_index": "Livestock_Density_Index"})
)

output_file = "HC_GeoCluster_Livestock_Density_Index.tsv"
