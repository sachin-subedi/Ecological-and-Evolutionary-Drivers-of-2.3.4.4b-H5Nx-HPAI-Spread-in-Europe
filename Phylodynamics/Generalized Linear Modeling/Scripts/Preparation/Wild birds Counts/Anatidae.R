...
counts_complete <- counts_year_cluster %>%
  tidyr::complete(GeoCluster = hc_clusters,
                  year = 2016:2025,
...
cluster_mean <- counts_complete %>%
  group_by(GeoCluster) %>%
  summarise(anatidae_counts = mean(total_count),
            .groups = "drop")
...
write_tsv(counts_complete,
          file.path(out_dir, "HC_Anatidae_counts_by_GeoCluster_year.tsv"))

write_tsv(cluster_mean,
          file.path(out_dir, "HC_Anatidae_anatidae_counts.tsv"))
