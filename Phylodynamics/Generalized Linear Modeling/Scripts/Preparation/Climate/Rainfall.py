...
mask2d = regions.mask(lon_or_obj=rr["longitude"], lat=rr["latitude"])

cluster_means = (
    xr.concat(
        [rr.where(mask2d == i).mean(dim=("latitude", "longitude")) for i in range(len(gdf))],
        dim="GeoCluster"
    )
...
season_country = (
    ...
    .agg(temp_sd=("rainfall", "std"))
    .groupby(["Country", "GeoCluster"], as_index=False)
    .agg(rainfall_seasonality=("temp_sd", "mean"))
)
...
print("Country-level rainfall mean:")
...
geo_out[["GeoCluster", "rainfall"]].to_csv(
