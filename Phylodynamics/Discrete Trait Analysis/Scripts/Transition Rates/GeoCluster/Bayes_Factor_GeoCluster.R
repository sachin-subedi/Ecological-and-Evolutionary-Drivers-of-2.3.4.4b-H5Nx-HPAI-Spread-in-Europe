...
workdir   <- "~/Library/CloudStorage/OneDrive-UniversityofGeorgia/EU_H5/may/data/Again/after_draft/pipeline/reproduce/rates/GeoCluster"
setwd(workdir)

burnin_pc <- 50
log_files <- c(
  "GeoCluster_rates_Subsample1_combined.log",
  "GeoCluster_rates_Subsample2_combined.log",
  "GeoCluster_rates_Subsample3_combined.log"
)
...
for (log_file in log_files) {
  ...
  subs_label <- gsub("GeoCluster_rates_(Subsample[0-9]+)_combined\\.log", "\\1", log_file)
  
  message("Processing: ", log_file, "  (", subs_label, ")")
  
  summaries <- process_log_file(log_file, burnin_pc = burnin_pc)
  
  out_file <- paste0("GeoCluster_bf_", subs_label, ".csv")
...
write_csv(all_summaries, "GeoCluster_bf_all_subsamples.csv")
