  ...
  df <- Region_ave_complete %>%
    ...
    mutate(
      Sink_lab = factor(
        ...
        levels = state_labels[valid_states],
        ordered = TRUE
      )
    )
...
for (i in seq_along(jump_files)) {
  ...
  csv_file <- file.path(
    out_dir,
    paste0("Region_AvgJumpsPerYear_", tag, ".csv")
  )
  ...
  png_file <- file.path(out_dir, paste0("time_Region_heatmap_", tag, ".png"))
  pdf_file <- file.path(out_dir, paste0("time_Region_heatmap_", tag, ".pdf"))
  
  ggsave(
    filename = png_file,
    plot     = final_plot_Region,
    width    = 14, height = 40, dpi = 600, units = "in", type = "cairo"
  )
  
  ggsave(
    filename = pdf_file,
    plot     = final_plot_Region,
    width    = 14, height = 28, units = "in", useDingbats = FALSE
  )
}
