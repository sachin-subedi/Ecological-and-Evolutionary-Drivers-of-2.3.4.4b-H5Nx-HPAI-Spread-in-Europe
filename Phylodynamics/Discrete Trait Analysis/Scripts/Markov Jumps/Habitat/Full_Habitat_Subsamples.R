...
src_cols <- list(
  ...
  "Forest"         = c(scales::alpha("#BCBD22", 0.25), "#BCBD22"),
  "Grassland"      = c(scales::alpha("#26A69A", 0.25), "#26A69A"),
  "Human_Modified" = c(scales::alpha("#56B4E9", 0.25), "#56B4E9"),
  "Marine"         = c(scales::alpha("#CC79A7", 0.25), "#CC79A7"),
  "Shrubland"      = c(scales::alpha("lightslategray", 0.25), "lightslategray"),
  "Urban"          = c(scales::alpha("#E6842A", 0.25), "#E6842A"),
  "Woodland"       = c(scales::alpha("#6B4C1B", 0.25), "#6B4C1B"),
  "Rock"           = c(scales::alpha("#7570B3", 0.25), "#7570B3"),
...
for (i in seq_along(jump_files)) {
  ...
  csv_file <- file.path(
    out_dir,
    paste0("Habitat_AvgJumpsPerYear_", tag, ".csv")
  )
  ...
  png_file <- file.path(out_dir, paste0("time_Habitat_heatmap_", tag, ".png"))
  pdf_file <- file.path(out_dir, paste0("time_Habitat_heatmap_", tag, ".pdf"))
  
  ggsave(
    filename = png_file,
    plot     = final_plot_habitat,
    width    = 14, height = 30, dpi = 600, units = "in", type = "cairo"
  )
  
  ggsave(
    filename = pdf_file,
    plot     = final_plot_habitat,
    width    = 14, height = 30, units = "in", useDingbats = FALSE
  )
}
